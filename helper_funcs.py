...
def load_data(subfolder='',t_cluster='2', n_cluster='1', generator=False):
...
#                 image = image / 127.5 - 1
    ...
    file_path=os.path.join(subfolder,'cluster'+t_cluster)

    trainA_path = os.path.join(file_path, n_cluster, 'trainCT')
    trainB_path = os.path.join(file_path, n_cluster, 'trainMRI')
    testA_path = os.path.join(file_path, n_cluster, 'testCT')
    testB_path = os.path.join(file_path, n_cluster, 'testMRI')
...
def save_model(opt, model, epoch):
    ...
    directory = os.path.join(opt['save_dir'],opt['date_time'])
